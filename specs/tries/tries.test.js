...
class Node {
  ...
  add(string) {
    ...
    for (let i = 0; i < this.children.length; i++) {
      ...
      if (child.value === value) {
        next ? child.add(next) : child.terminus = true;
        return;
      }
    }
  ...
  _complete(search, built, suggestions) {
    if (suggestions.length >= 3 || (search && search[0] !== this.value)) return suggestions;

    if (this.terminus) suggestions.push(built + this.value);
